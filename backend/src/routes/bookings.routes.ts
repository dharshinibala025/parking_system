...
function calculateDurationInHours(startTimeStr: string, endTimeStr: string, bookingDate: string): number {
  try {
...
router.post('/', authenticateUser, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const lotDoc = await db.collection('parkingLots').doc(parkingLotId).get()
    if (!lotDoc.exists) {
    ...
    const slotDoc = await db.collection('parkingSlots').doc(parkingSlotId).get()
    if (!slotDoc.exists) {
    ...
    existingBookingsSnapshot.forEach((doc: any) => {
      const b = doc.data()
...
router.patch('/:id/cancel', authenticateUser, async (req: AuthRequest, res: Response) => {
  try {
    const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id
